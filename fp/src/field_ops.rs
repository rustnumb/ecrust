...
macro_rules! ref_field_impl {
    ...
    (impl<$F:ident : $($Bounds:tt)+> $Ty:ty { $($body:tt)* }) => {
        impl<$F: $($Bounds)+> $Ty
        where
            $F: $crate::field_ops::FieldOps,
            for<'a, 'b> &'a $F: std::ops::Add<&'b $F, Output = $F>,
...
macro_rules! ref_field_trait_impl {
    ...
    (impl<$F:ident : $($Bounds:tt)+> $Trait:ident for $Ty:ty { $($body:tt)* }) => {
        impl<$F: $($Bounds)+> $Trait for $Ty
        where
            $F: $crate::field_ops::FieldOps,
            for<'a, 'b> &'a $F: std::ops::Add<&'b $F, Output = $F>,
            for<'a, 'b> &'a $F: std::ops::Sub<&'b $F, Output = $F>,
...
macro_rules! ref_field_trait_impl_path {
    ...
    (impl<$F:ident : $($Bounds:tt)+> ($Trait:path) for $Ty:ty { $($body:tt)* }) => {
        impl<$F: $($Bounds)+> $Trait for $Ty
        where
            $F: $crate::field_ops::FieldOps,
            for<'a, 'b> &'a $F: std::ops::Add<&'b $F, Output = $F>,
...
macro_rules! ref_field_fn {
    ...
    (fn $name:ident<$F:ident : $($Bounds:tt)+> ( $($args:tt)* ) -> $Ret:ty $body:block) => {
        fn $name<$F: $($Bounds)+>($($args)*) -> $Ret
        where
            $F: $crate::field_ops::FieldOps,
            for<'a, 'b> &'a $F: std::ops::Add<&'b $F, Output = $F>,
            for<'a, 'b> &'a $F: std::ops::Sub<&'b $F, Output = $F>,
...
macro_rules! ref_field_fns {
    ...
    (
        ...
        $(#[$meta])*
        $crate::ref_field_fn! {
            fn $name<$F: $($Bounds)+>($($args)*) -> $Ret $body
        }
        $crate::ref_field_fns! { $($rest)* }
    };
}
